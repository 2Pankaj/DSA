...
void buildsegtree(vector<int> &vec, int s, int e, int st_idx){
    ...
    st[st_idx] = min(st[2*st_idx],st[2*st_idx+1]);
}
int mx(vector<int> &st, int s, int e, int st_idx,int qs,int qe){
    if(qs>e or qe <s) return 1e9;
    if(s>=qs and e<= qe) return st[st_idx];
    int mid=  s+  (e-s)/2;
    int x = mx(st,s,mid,2*st_idx,qs,qe);
    int y = mx(st,mid+1,e,2*st_idx+1,qs,qe);
    return min(x,y);
}
